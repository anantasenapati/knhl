...
VedaMantraCol = 9
VedaPadaPathCol = 11
VedaMantraSwaraRohitCol = 10
VedaPadaPathSwaraRohitCol = 12
VedaSanskritCommentaryCol = 21
VedaHindiBhasarthaCol = 19
HindiBhaVarthaCol = 20
RishiCol = 13
DevataCol = 14
ChandaCol = 15
SuktamCol = 16
SanskritManraVisayaCol = 17
HindiMantraVisayaCol = 18
bookName = 'Book'
title = ''
target = []
...
def getTitle(row):
    ...
    chapterName = str(row[chapterNameCol] if row[chapterNameCol] != 'nan' else "")
    sectionName = str(row[sectionNameCol] if row[sectionNameCol] != 'nan' else "")
...
def makeMantraList(wks):
        ...
        for row in wks:
            ...
            if (Suktam != prevSuktam):
                ...
                mantraList=[ '[[' + getTitle(row) +"|" + "Suktam " + str(Kanda) + '.' + str(Suktam) + " Mantra Index]]",
                     "[[" + getTitle(row) + "|" + str(mantraNo) + "]]"] # start a new list
                prevSuktam = Suktam
            else:
                if (not mantraList):
                        mantraList=[ "[[" + getTitle(row) +"|" + "Suktam " + str(Kanda) + '.' + str(Suktam) + " Mantra Index]]"] # start a new list
...
def makeLists(wks):
    ...
    for row in wks:
        ...
        if ( not chapterList):
            #chapterList.append(str(row[chapterNameCol]) or ("'''Kanda List'''"))                
            chapterList.append(("'''Kanda Index'''"))                
        ...
        if ( (chapter or bookName)+"]]" != chapterList[-1].rsplit('|',1)[1]): # if current chapter is not the same as the last item in chapterList
            ...
            sectionList=[ "[[" + getTitle(row) + "|" + "Kanda " + chapter + " Suktam Index]]", 
                    "[[" + getTitle(row) + "|" + chapter + (('.' + section) if chapter else section)+ "]]"] # start a new list
        else:
            if (not sectionList): 
                    sectionList=[ "[[" + getTitle(row) + "|" + "Kanda " + chapter + " Suktam Index]]"] # start a new list
...
def startPageHeader( id, pageName):
    ...
    if id == 'VedaMantra':
        ...
        pageHeader += '{{VedaSanskritCommentary|\n' # template
    elif id == 'VedaHindiCommentary':
        pageHeader += '{{VedaHindiCommentary|\n' # template
    elif id == 'VedaMeta':
        pageHeader += '{{VedaMeta|\n' # template
    else:
        pageHeader += '<div class="siva_container">\n'  # div for sloka text for transliteration
...
#### implement >>> l = [None, *l, None] and then for prev, cur, nxt in zip(l, l[1:], l[2:]):
    ...
    #for row in [*wks][skipCount:]:
        ...
        if prevVerseNo is not None and prevVerseNo != "Verse Number":
            target.append("<div style='text-align: left;float:left;width:33%;'>[["+"AtharvaVeda "+prevVerseNo+"|←Previous]]</div>\n")
        else:    
            target.append("<div style='text-align: left;float:left;width:33%;'>"+verseNo+"</div>\n")
        target.append("<div style='text-align: center;float:left;width:33%;'>'''"+"AtharvaVeda "+verseNo+"'''</div>\n")
        if nxtVerseNo is not None:
            target.append("<div style='text-align: right;float:left;width:33%;'>[["+"AtharvaVeda "+nxtVerseNo+"|Next→]]</div>\n")
        target.append("\n")
        ...
        VedaSanskritCommentary.append('{{ns}}' + row[VedaSanskritCommentaryCol].replace('=', '<nowiki>=</nowiki>') + '|\n')
        VedaSanskritCommentary.append('}}\n</div>\n')

        VedaHindiCommentary.append('{{ns}}' + row[HindiMantraVisayaCol].replace('=', '<nowiki>=</nowiki>') + '|\n')
        VedaHindiCommentary.append('{{ns}}' + row[VedaHindiBhasarthaCol].replace('=', '<nowiki>=</nowiki>') + '|\n')
        ...
        VedaMeta.append('{{ns}}'+row[SuktamCol])
        VedaMeta.append('}}\n</div>\n')
        VedaMeta.append('</div>\n')
