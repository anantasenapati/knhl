...
VedaMantraCol = 15
VedaPadaPathCol = 16
VedaMantraSwaraRohitCol = 17
VedaPadaPathSwaraRohitCol = 18

SanskritAnvayahCol = 25
SanskritManraVisayaCol = 26
SanskritPadarthaCol = 27
SanskritBhaVarthaCol = 28

HindiMantraVisayaCol = 29
HindiPadarthaCol = 30
HindiBhaVarthaCol = 31
...
def makeMantraList(wks):
        ...
        for row in wks:
            ...
            if (Suktam != prevSuktam):
                ...
                mantraList=[ '[[' + getTitle(row) +"|" + "Suktam " + str(Mandalam) + '.' + str(Suktam) + " Mantra Index]]",
                     "[[" + getTitle(row) + "|" + str(mantraNo) + "]]"] # start a new list
                prevSuktam = Suktam
            else:
                if (not mantraList):
                        mantraList=[ "[[" + getTitle(row) +"|" + "Suktam " + str(Mandalam) + '.' + str(Suktam) + " Mantra Index]]"] # start a new list
...
def makeLists(wks):
    ...
    for row in wks:
        ...
        if ( (chapter or bookName)+"]]" != chapterList[-1].rsplit('|',1)[1]): # if current chapter is not the same as the last item in chapterList
            ...
            sectionList=[ "[[" + getTitle(row) + "|" + "Mandalam " + chapter + " Suktam Index]]", 
                    "[[" + getTitle(row) + "|" + chapter + (('.' + section) if chapter else section)+ "]]"] # start a new list
        else:
            if (not sectionList): 
                    sectionList=[ "[[" + getTitle(row) + "|" + "Mandalam " + chapter + " Suktam Index]]"] # start a new list
...
#### implement >>> l = [None, *l, None] and then for prev, cur, nxt in zip(l, l[1:], l[2:]):
    ...
    #for row in [*wks][skipCount:]:
        ...
        if prevVerseNo is not None and prevVerseNo != "Verse Number":
            target.append("<div style='text-align: left;float:left;width:33%;'>[["+"RigVeda "+prevVerseNo+"|←Previous]]</div>\n")
        else:    
            target.append("<div style='text-align: left;float:left;width:33%;'>"+verseNo+"</div>\n")
        target.append("<div style='text-align: center;float:left;width:33%;'>'''"+"RigVeda "+verseNo+"'''</div>\n")
        if nxtVerseNo is not None:
            target.append("<div style='text-align: right;float:left;width:33%;'>[["+"RigVeda "+nxtVerseNo+"|Next→]]</div>\n")
        target.append("\n")
