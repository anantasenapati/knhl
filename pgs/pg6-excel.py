...
def makeLists(wks):
    ...
    for row in wks:
        ...
        if ( not chapterList):
            chapterList.append(str(row[chapterNameCol]) or ('Chapters'))                
...
def startNewPage(row):
    ...
    global bookName
    stopPageFooter()
    printFile("verses/"+ bookName + str(prevChapter) + '-' + prevSection, target)
    prevSection = section
    prevChapter = chapter
    target.clear()
    ...
    target.append("{{-start-}}\n")
    appendPageLists()
    target.append (startPageHeader( getTitle(row)))
...
def main(argv):
    ...
    if GOOGLE_SHEETS_FILE_ID:
        ...
        import requests
        df = pd.read_csv(csvFile, header=None, encoding='utf8', skiprows=None)
        ...
        wks = pd.read_excel(xcelFile, sheet_name=0, header=None, skiprows=None).values
        #wks= df
    ...
    for row in [*wks][skipCount:]:
        ...
        for e in lines:
            ...
            if (e == '{{ns}}StartNewPage'):         
                if csvFile:
                    prevChapter = newCsvPage
                    newCsvPage += 1
                    startNewPage(row)   # 'StartNewPage' tag in csv file starts a new page
                    continue    
